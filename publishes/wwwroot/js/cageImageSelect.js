...
var csw = (canvas.width - (dimOfCanv.frames + 1) * mr - mr) / (dimOfCanv.frames + 1); // cage single width
var csh = (canvas.height - (dimOfCanv.rows * dimOfCanv.levels - 1) * mr - mBtwRows * (dimOfCanv.rows - 1)) / (dimOfCanv.rows * dimOfCanv.levels); // cage single height

var rd = (csh + mr) * dimOfCanv.levels - (csh) + mBtwRows; //distance btw rows
//console.log(csh, canvas.height, dimOfCanv.rows, dimOfCanv.levels, rd);
var fonNumbSize = csh * 0.7;
var fontNumb = fonNumbSize + 'px serif';
var fontLiversSize = (csh * 0.3);
var fontLivers = fontLiversSize + 'px serif';
ctx.font = fontNumb;
...
function getMousePosition(canvas, event) {
    ...
    //    "Coordinate y: " + y);
    let cel = cellsArray.find(c => c.xc <= x && c.xpc >= x && c.yc <= y && c.ypc >= y);
    //console.log(cel);
    if (typeof cel !== 'undefined') {
        var CageListHTML = document.getElementById('CageList').options[parseInt(cel.id)].innerHTML;
        var CageListValue = document.getElementById('CageList').options[parseInt(cel.id)].value;

        if (typeof cel !== 'undefined' || typeof CageList !== 'undefined') {
            var CageListHTML = document.getElementById('CageList').options[parseInt(cel.id)].innerHTML;
            var CageListValue = document.getElementById('CageList').options[parseInt(cel.id)].value;
            //console.log(CageListHTML + "  " + CageListValue);
            document.getElementById('CageList').options[0].innerHTML = CageListHTML;
            document.getElementById('CageList').options[0].value = CageListValue;
            //CageListField.value = cel.id;//console.log("cel id:" + cel.id);
            //CageListField.text = cel.id;
        }
