...
function drawPedigree(partId, rabId) {
    //console.log(partId + "-" + rabId);
    var canvas = document.getElementById('pedigree_drawing-' + partId+"-"+rabId);
    ...
    var descentObj = JSON.parse($("#pedigree_data-" + partId + "-" + rabId).html());
   // var descentObj=dataToDraw;
    defineDimentions(descentObj, 0, 0);
    dimOfCanv.columns = dimOfCanv.goLeft + dimOfCanv.goRight;
   if (dimOfCanv.goLeft + dimOfCanv.goRight > 3) {
        canvas.height = 450;
        canvas.width = 450;
    }
    else {
    ...
    ctx.fillText(rabId, startPosCelX + CSW - mrT, startPosCelY);
